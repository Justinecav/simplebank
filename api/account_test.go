...
import (
	...
	mockdb "github.com/Justinecav/simplebank.git/db/mock"
	...
	testCases := []struct {
	...
	}{
		{
			...
			checkResponse: func(t *testing.T, recorder *httptest.ResponseRecorder) {
				...
				requireBodyMatchedAccount(t, recorder.Body, account)
			},
		},
		{
		...
		{
			name:         "badRequest server error response",
			accountID:    0,
			authUsername: user.Username,
			buildStubs: func(store *mockdb.MockStore) {
				//build stubs
...
func requireBodyMatchedAccount(t *testing.T, body *bytes.Buffer, account db.Account) {
